...
exports.createSrvEditWindow = function createSrvEditWindow(parentWindow, type, contents) {
  ...
  if (!win || win === null) {
    ...
    win.loadFile(path.join(__dirname, "../views/services/editServices.html"));
    //win.openDevTools();
    ...
    **/
    ...
    ipcMain.on("dismiss-form-window", (event, args) => {
      if(win) win.close();
      ipcMain.removeHandler("edit-service"); // remove existing handler
    })
